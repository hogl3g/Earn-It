...
import { loadTeamMetrics, compareTeams, calculateStrengthAdjustment, calculateConfidenceAdjustment, getOffensiveProfile } from '../lib/team_metrics_integration.js';
...
function applyMetricsToPickExample() {
  ...
  const withStrength = Array.from(metrics.values()).filter((t: any) => t.strength_rating).length;
  const withOffensive = Array.from(metrics.values()).filter((t: any) => t.offensive_rating).length;
  const withBoth = Array.from(metrics.values()).filter((t: any) => t.strength_rating && t.offensive_rating).length;
